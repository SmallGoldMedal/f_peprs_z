  ...
  <div class="admin-container">
    ...
    <el-tabs v-model="activeTab" type="border-card">
      ...
      <el-tab-pane label="账号管理" name="accounts">
        <div class="toolbar">
          <el-input v-model="accountQuery.accountName" placeholder="账号名" style="width: 150px; margin-right: 10px" />
          <el-select v-model="accountQuery.role" placeholder="角色" clearable style="width: 100px; margin-right: 10px">
            <el-option label="普通用户" :value="0" />
            <el-option label="管理员" :value="1" />
          </el-select>
          <el-select v-model="accountQuery.accountState" placeholder="状态" clearable style="width: 100px; margin-right: 10px">
            <el-option label="正常" :value="0" />
            <el-option label="注销" :value="1" />
          </el-select>
          <el-button type="primary" @click="fetchAccounts">查询</el-button>
          <el-button @click="resetAccountQuery">重置</el-button>
        </div>
        <el-table :data="accountList" border stripe v-loading="loadingAccounts">
          ...
          <el-table-column prop="accountState" label="状态" width="80">
            <template #default="{ row }">
              <el-tag :type="row.accountState === 0 ? 'success' : 'danger'">{{ row.accountState === 0 ? '正常' : '注销' }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="100">
            <template #default="{ row }">
              <el-button v-if="row.accountState === 0" type="danger" link @click="updateAccountState(row.id, 1)">禁用</el-button>
              <el-button v-else type="success" link @click="updateAccountState(row.id, 0)">启用</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
      ...
      <el-tab-pane label="用户信息管理" name="users">
        <div class="toolbar">
          <el-input v-model="userQuery.username" placeholder="昵称" style="width: 150px; margin-right: 10px" />
          <el-select v-model="userQuery.gender" placeholder="性别" clearable style="width: 100px; margin-right: 10px">
            <el-option label="男" :value="1" />
            <el-option label="女" :value="2" />
          </el-select>
          <el-select v-model="userQuery.userTarget" placeholder="运动目标" clearable style="width: 120px; margin-right: 10px">
        ...
        <el-table :data="userList" border stripe v-loading="loadingUsers">
          ...
          <el-table-column prop="userTarget" label="运动目标" min-width="100">
            <template #default="{ row }">{{ formatTargetMask(row.userTarget) }}</template>
          </el-table-column>
          <el-table-column prop="userEnvironment" label="运动环境" min-width="100">
            <template #default="{ row }">{{ formatEnvMask(row.userEnvironment) }}</template>
          </el-table-column>
      ...
      <el-tab-pane label="方案管理" name="plans">
        ...
        <div class="toolbar">
          ...
          <el-select v-model="planQuery.planStatus" placeholder="状态" clearable style="width: 100px; margin-right: 10px">
            <el-option label="推荐" :value="0" />
            <el-option label="被选择" :value="1" />
        ...
        <el-table :data="planList" border stripe v-loading="loadingPlans">
          ...
          <el-table-column prop="planStartDate" label="开始日期" width="100" />
          <el-table-column prop="endDate" label="结束日期" width="100" />
          <el-table-column prop="planStatus" label="状态" width="80">
            <template #default="{ row }">{{ row.planStatus === 0 ? '推荐' : row.planStatus === 1 ? '被选择' : '已完成' }}</template>
          </el-table-column>
          <el-table-column prop="adopted" label="是否采纳" width="80">
      ...
      <el-tab-pane label="伤病记录管理" name="injuries">
        <div class="toolbar">
          <el-input v-model="injuryQuery.injuryName" placeholder="伤病名称" style="width: 150px; margin-right: 10px" />
          <el-input v-model="injuryQuery.userId" placeholder="用户ID" style="width: 100px; margin-right: 10px" />
          <el-select v-model="injuryQuery.injurySeverity" placeholder="严重程度" clearable style="width: 100px; margin-right: 10px">
        ...
        <el-table :data="injuryList" border stripe v-loading="loadingInjuries">
          ...
          <el-table-column prop="injuryBodyPart" label="伤病部位" width="100" />
          <el-table-column prop="injurySeverity" label="严重程度" width="80">
            <template #default="{ row }">
              <el-tag :type="row.injurySeverity === 0 ? 'info' : row.injurySeverity === 1 ? 'warning' : 'danger'">
                {{ row.injurySeverity === 0 ? '轻度' : row.injurySeverity === 1 ? '中度' : '重度' }}
              </el-tag>
            </template>
          </el-table-column>
      ...
      <el-tab-pane label="运动记录管理" name="workouts">
        <div class="toolbar">
          ...
          <el-date-picker v-model="workoutQuery.completionDateStart" type="date" placeholder="开始日期" value-format="YYYY-MM-DD" style="width: 130px; margin-right: 10px" />
          <el-date-picker v-model="workoutQuery.completionDateEnd" type="date" placeholder="结束日期" value-format="YYYY-MM-DD" style="width: 130px; margin-right: 10px" />
        ...
        <el-table :data="workoutList" border stripe v-loading="loadingWorkouts">
          ...
          <el-table-column prop="exerciseName" label="运动项目" min-width="120" />
          ...
          <el-table-column prop="workoutRecordStatus" label="状态" width="80">
            <template #default="{ row }">
              <el-tag :type="row.workoutRecordStatus === 0 ? 'success' : 'info'">
                {{ row.workoutRecordStatus === 0 ? '已完成' : row.workoutRecordStatus === 1 ? '跳过' : '未完成' }}
              </el-tag>
            </template>
          </el-table-column>
    ...
    <el-dialog v-model="exerciseDialogVisible" :title="exerciseDialogTitle" width="600px" destroy-on-close>
      <el-form :model="exerciseForm" :rules="exerciseRules" ref="exerciseFormRef" label-width="100px">
        ...
        <el-form-item label="每组次数" prop="exerciseReps">
          <el-input-number v-model="exerciseForm.exerciseReps" :min="1" />
        </el-form-item>
        <el-form-item label="距离(km)" prop="exerciseDistance">
          <el-input-number v-model="exerciseForm.exerciseDistance" :min="0" :precision="2" />
        </el-form-item>
        <el-form-item label="频率" prop="exerciseFrequency">
          <el-input-number v-model="exerciseForm.exerciseFrequency" :min="1" />
        </el-form-item>
        ...
        <el-form-item label="建议时长(分)" prop="exerciseDurationMin">
          <el-input-number v-model="exerciseForm.exerciseDurationMin" :min="1" />
        </el-form-item>
        <el-form-item label="默认组数" prop="exerciseSets">
          <el-input-number v-model="exerciseForm.exerciseSets" :min="1" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const exerciseForm = ref({
  id: null, exerciseName: '', categoryMask: null, difficulty: null, targetMuscle: '',
  secondaryMuscle: '', exerciseReps: null, exerciseDistance: null, exerciseFrequency: null,
  intensity: null, suitableEnvMask: [], requiredEquipment: '', cautions: '',
  exerciseDurationMin: null, exerciseSets: 3
})
const exerciseRules = {
...
const fetchExercises = async () => {
  ...
  } catch (error) { ElMessage.error('获取运动项目失败') }
...
const renderCategoryChart = () => {
  ...
  const data = Object.entries(counts).map(([mask, count]) => ({ name: categories[mask], value: count })).filter(d => d.value > 0)
...
const openExerciseDialog = (row = null) => {
  if (row) {
    ...
    exerciseForm.value = { id: row.id, exerciseName: row.exerciseName, categoryMask: row.categoryMask, difficulty: row.difficulty, targetMuscle: row.targetMuscle || '', secondaryMuscle: row.secondaryMuscle || '', exerciseReps: row.exerciseReps, exerciseDistance: row.exerciseDistance, exerciseFrequency: row.exerciseFrequency, intensity: row.intensity, suitableEnvMask: row.suitableEnvMaskArr || [], requiredEquipment: row.requiredEquipment || '', cautions: row.cautions || '', exerciseDurationMin: row.exerciseDurationMin, exerciseSets: row.exerciseSets || 3 }
  } else {
    exerciseDialogTitle.value = '新增运动'
    exerciseForm.value = { id: null, exerciseName: '', categoryMask: null, difficulty: null, targetMuscle: '', secondaryMuscle: '', exerciseReps: null, exerciseDistance: null, exerciseFrequency: null, intensity: null, suitableEnvMask: [], requiredEquipment: '', cautions: '', exerciseDurationMin: null, exerciseSets: 3 }
  }
  exerciseDialogVisible.value = true
}

const submitExercise = async () => {
  try { await exerciseFormRef.value.validate() } catch { return }
  ...
  try {
    ...
    const payload = { exerciseName: exerciseForm.value.exerciseName, categoryMask: exerciseForm.value.categoryMask, difficulty: exerciseForm.value.difficulty, intensity: exerciseForm.value.intensity, targetMuscle: exerciseForm.value.targetMuscle, secondaryMuscle: exerciseForm.value.secondaryMuscle, exerciseDurationMin: exerciseForm.value.exerciseDurationMin, exerciseSets: exerciseForm.value.exerciseSets, exerciseReps: exerciseForm.value.exerciseReps, exerciseDistance: exerciseForm.value.exerciseDistance, exerciseFrequency: exerciseForm.value.exerciseFrequency, suitableEnvMask: envMask, requiredEquipment: exerciseForm.value.requiredEquipment, cautions: exerciseForm.value.cautions }
    let res
    if (exerciseForm.value.id) res = await request.put(`/exercise/${exerciseForm.value.id}`, payload)
    else res = await request.post('/exercise', payload)
    if (res.data.code === 200) {
  ...
  finally { exerciseSubmitLoading.value = false }
}

const deleteExercise = async (id) => {
  try {
    await ElMessageBox.confirm('确定删除该运动项目吗？', '提示', { type: 'warning' })
    const res = await request.delete(`/exercise/${id}`)
    if (res.data.code === 200) { ElMessage.success('删除成功'); await fetchExercises() }
    else ElMessage.error(res.data.message)
...
const exclusionRules = { injuryKeyword: [{ required: true, message: '请输入伤病关键词', trigger: 'blur' }], exerciseId: [{ required: true, message: '请选择运动项目', trigger: 'change' }] }

const fetchExclusions = async () => {
  loadingExclusions.value = true
  try {
    const res = await request.get('/exclusion/all')
    if (res.data.code === 200) exclusionList.value = res.data.data || []
    else ElMessage.error(res.data.message)
  } catch (error) { ElMessage.error('获取禁忌规则失败') }
  finally { loadingExclusions.value = false }
}
...
const openExclusionDialog = (row = null) => {
  if (row) {
    ...
    exclusionForm.value = { id: row.id, injuryKeyword: row.injuryKeyword, exerciseId: row.exerciseId, severityLimit: row.severityLimit }
  } else {
...
const submitExclusion = async () => {
  try { await exclusionFormRef.value.validate() } catch { return }
  exclusionSubmitLoading.value = true
  try {
    let res
    if (exclusionForm.value.id) res = await request.put(`/exclusion/${exclusionForm.value.id}`, exclusionForm.value)
    else res = await request.post('/exclusion', exclusionForm.value)
    if (res.data.code === 200) {
  ...
  finally { exclusionSubmitLoading.value = false }
}

const deleteExclusion = async (id) => {
  try {
    await ElMessageBox.confirm('确定删除该规则吗？', '提示', { type: 'warning' })
    const res = await request.delete(`/exclusion/${id}`)
    if (res.data.code === 200) { ElMessage.success('删除成功'); await fetchExclusions() }
    else ElMessage.error(res.data.message)
  } catch (error) { if (error !== 'cancel') ElMessage.error('删除失败') }
}
...
const accountQuery = reactive({ page: 1, size: 10, accountName: '', role: null, accountState: null })

const fetchAccounts = async () => {
  loadingAccounts.value = true
  try {
    const res = await request.post('/admin/account/query', accountQuery)
    if (res.data.code === 200) {
      accountList.value = res.data.data.records || []
      accountTotal.value = res.data.data.total || 0
    } else ElMessage.error(res.data.message)
  } catch (error) { ElMessage.error('获取账号列表失败') }
  finally { loadingAccounts.value = false }
}

const resetAccountQuery = () => {
  accountQuery.accountName = ''; accountQuery.role = null; accountQuery.accountState = null; accountQuery.page = 1
  fetchAccounts()
}

const updateAccountState = async (accountId, newState) => {
  try {
    const res = await request.put('/admin/account/state', { accountId, accountState: newState })
    if (res.data.code === 200) { ElMessage.success('状态更新成功'); fetchAccounts() }
    else ElMessage.error(res.data.message)
  } catch (error) { ElMessage.error('操作失败') }
}
...
const userQuery = reactive({ page: 1, size: 10, username: '', gender: null, userTarget: null })
...
const fetchUsers = async () => {
  ...
  try {
    const res = await request.post('/admin/user/query', userQuery)
    if (res.data.code === 200) {
...
const resetUserQuery = () => {
  userQuery.username = ''; userQuery.gender = null; userQuery.userTarget = null; userQuery.page = 1
  fetchUsers()
}
...
const planQuery = reactive({ page: 1, size: 10, planName: '', userId: null, planStatus: null })

const fetchPlans = async () => {
  loadingPlans.value = true
  try {
    const res = await request.post('/admin/plan/query', planQuery)
    if (res.data.code === 200) {
      planList.value = res.data.data.records || []
      planTotal.value = res.data.data.total || 0
      renderPlanStatusChart()
    } else ElMessage.error(res.data.message)
  } catch (error) { ElMessage.error('获取方案列表失败') }
  finally { loadingPlans.value = false }
}

const renderPlanStatusChart = () => {
  if (!planStatusChartRef.value) return
  const statuses = { 0: '推荐', 1: '被选择', 2: '已完成' }
  const counts = { 0: 0, 1: 0, 2: 0 }
  planList.value.forEach(plan => { counts[plan.planStatus]++ })
  const data = Object.entries(counts).map(([status, count]) => ({ name: statuses[status], value: count }))
...
const resetPlanQuery = () => {
  planQuery.planName = ''; planQuery.userId = null; planQuery.planStatus = null; planQuery.page = 1
  fetchPlans()
}
...
const deletePlan = async (planId) => {
  try {
    ...
    if (res.data.code === 200) { ElMessage.success('删除成功'); fetchPlans() }
    else ElMessage.error(res.data.message)
  } catch (error) { if (error !== 'cancel') ElMessage.error('删除失败') }
}
...
const injuryQuery = reactive({ page: 1, size: 10, injuryName: '', userId: null, injurySeverity: null, status: null })
...
const fetchInjuries = async () => {
  ...
  try {
    const res = await request.post('/admin/injury/query', injuryQuery)
    if (res.data.code === 200) {
      injuryList.value = res.data.data.records || []
      injuryTotal.value = res.data.data.total || 0
    } else ElMessage.error(res.data.message)
  } catch (error) { ElMessage.error('获取伤病记录失败') }
  finally { loadingInjuries.value = false }
}

const resetInjuryQuery = () => {
  injuryQuery.injuryName = ''; injuryQuery.userId = null; injuryQuery.injurySeverity = null; injuryQuery.status = null; injuryQuery.page = 1
  fetchInjuries()
}
...
const deleteInjury = async (id) => {
  try {
    ...
    if (res.data.code === 200) { ElMessage.success('删除成功'); fetchInjuries() }
    else ElMessage.error(res.data.message)
  } catch (error) { if (error !== 'cancel') ElMessage.error('删除失败') }
}
...
const workoutQuery = reactive({ page: 1, size: 10, userId: null, planId: null, completionDateStart: null, completionDateEnd: null, workoutRecordStatus: null })

const fetchWorkoutRecords = async () => {
  loadingWorkouts.value = true
  try {
    const res = await request.post('/admin/record/query', workoutQuery)
    if (res.data.code === 200) {
      workoutList.value = res.data.data.records || []
      workoutTotal.value = res.data.data.total || 0
    } else ElMessage.error(res.data.message)
  } catch (error) { ElMessage.error('获取运动记录失败') }
  finally { loadingWorkouts.value = false }
}

const resetWorkoutQuery = () => {
  workoutQuery.userId = null; workoutQuery.planId = null; workoutQuery.completionDateStart = null; workoutQuery.completionDateEnd = null; workoutQuery.workoutRecordStatus = null; workoutQuery.page = 1
  fetchWorkoutRecords()
}
...
const deleteWorkoutRecord = async (id) => {
  try {
    ...
    if (res.data.code === 200) { ElMessage.success('删除成功'); fetchWorkoutRecords() }
    else ElMessage.error(res.data.message)
  } catch (error) { if (error !== 'cancel') ElMessage.error('删除失败') }
}
...
<style scoped>
.admin-container { padding: 20px; background: #fff; border-radius: 16px; }
